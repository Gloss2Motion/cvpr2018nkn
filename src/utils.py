...
def get_floor(sk_in):
  h_root = sk_in[0, 0, 1]
  return np.min([
      h_root + sk_in[0, 10, 1] + sk_in[0, 11, 1] + sk_in[0, 12, 1] +
      sk_in[0, 13, 1], h_root + sk_in[0, 6, 1] + sk_in[0, 7, 1] +
      sk_in[0, 8, 1] + sk_in[0, 9, 1]
  ])
      ...
      for cfile in files:
        ...
        if tgtskel.shape[0] >= min_steps + 1:
          if not ("Claire" in inp and "Warrok" in tgt):
            count += 1
          inpanim, inpnames, inpftime = BVH.load(data_path + inp + "/" +
                                                 cfile[:-8] + ".bvh")

          tgtanim, tgtnames, tgtftime = BVH.load(data_path + tgt + "/" +
                                                 cfile[:-8] + ".bvh")

          gtanim = tgtanim.copy()

          ibvh_file = open(data_path + inp + "/" + cfile[:-8] +
                           ".bvh").read().split("JOINT")
          ibvh_joints = [
              f.split("\n")[0].split(":")[-1].split(" ")[-1]
          ...
          tbvh_file = open(data_path + tgt + "/" + cfile[:-8] +
                           ".bvh").read().split("JOINT")
          tbvh_joints = [
              f.split("\n")[0].split(":")[-1].split(" ")[-1]
          ...
          for s in range(num_samples):
            ...
            inpanims.append([
                inpanim.copy()[s * max_steps:(s + 1) * max_steps], inpnames,
                inpftime
            ])
            tgtanims.append([
                tgtanim.copy()[s * max_steps:(s + 1) * max_steps], tgtnames,
                tgtftime
            ])
            gtanims.append([
                gtanim.copy()[s * max_steps:(s + 1) * max_steps], tgtnames,
                tgtftime
            ])
          ...
          if not inseq.shape[0] % max_steps == 0:
            inpjoints.append(ito_keep)
            tgtjoints.append(tto_keep)
            inpanims.append([inpanim.copy()[-max_steps:], inpnames, inpftime])
            tgtanims.append([tgtanim.copy()[-max_steps:], tgtnames, tgtftime])
